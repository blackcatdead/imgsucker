	...
	if request.method == 'POST':
		...
		if title != 'Page Not Found':
			...
			if my_file.is_file():
				...
				wp = Wallpaper(link=link, source='https://wallpaperscraft.com'+source, owner=owner, owner_license=owner_license, owner_link=owner_link, category=cat, grab_host='wallpaperscraft.com', uploader= User.objects.order_by('?').first(), status=0)
				wp.save()
